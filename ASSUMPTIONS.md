...
- A reported Yes/No is held for `dispute_window` and then pays the winner the full notional; Invalid refunds each poster its own chunk; Disputed holds until adjudication or `unwind_timeout` refunds everyone → money moves once per leg, is never reversed, and always has an exit; filing is free, so the losing side can buy delay (bond is future work).
- `Locked` has no timer → escrow on a silent oracle is held until an outcome arrives; `Reported` and `Disputed` do (`dispute_window`, `unwind_timeout`), and only the requester or a maker locked into the request may file.
...
- Maker collateral at submit is its escrow side at the quote price for the full leg notional, reserved per quote with no netting across requests → quotes are firm and fully collateralized; `size` above notional reserves nothing extra; a maker's capital is committed until that quote is cancelled, expires while `Open`, loses, or its request fails, and a quote it cannot fund is `402`, so capital efficiency is traded for a lock batch that can never fail on the maker's side.
...
- The engine is one actor over a bounded queue → mutations are serial, so accept/expiry/cancel cannot race and two quotes against the same free balance are checked in order (the second is `402`, never both); throughput is bounded by design and a full queue back-pressures handlers instead of returning 503.
- Ledger and clock are in-memory and the oracle is whoever calls resolve; nothing persists → a restart loses every request and balance.
