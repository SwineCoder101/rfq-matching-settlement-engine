...
use common::{ACCEPT_WINDOW_SECS, TestVenue, assert_quote_states, bal, fixture, id_of, leg_ids, ts};
use uuid::Uuid;
...
async fn full_lifecycle_two_legs_settles_yes() {
    ...
    let q_a_mm1 = v.quote_ok(mm1, &request_id, &leg_a, 4_000, 1_000, expires).await; // 600
    assert_eq!(v.balances(mm1).await, bal(9_400, 600, 0));
    let q_a_mm2 = v.quote_ok(mm2, &request_id, &leg_a, 3_500, 1_000, expires).await; // 650, better for a Yes-buyer
    assert_eq!(v.balances(mm2).await, bal(9_350, 650, 0));
    // Leg B (SellYes): MM is the Yes-buyer and reserves p * n.
    let q_b_mm1 = v.quote_ok(mm1, &request_id, &leg_b, 6_000, 2_000, expires).await; // 1_200
    assert_eq!(v.balances(mm1).await, bal(8_200, 1_800, 0));
    let q_b_mm2 = v.quote_ok(mm2, &request_id, &leg_b, 6_500, 2_500, expires).await; // 1_300, better for a Yes-seller
    ...
    let q_a_mm1_cheap = v.quote_ok(mm1, &request_id, &leg_a, 3_000, 1_000, expires).await; // 700
    assert_eq!(v.balances(mm1).await, bal(7_500, 2_500, 0));
    assert_eq!(v.cancel_quote(mm1, &q_a_mm1_cheap).await.0, StatusCode::NO_CONTENT);
    assert_eq!(v.balances(mm1).await, bal(8_200, 1_800, 0));
    ...
    let (status, settled) = v.resolve(v.oracle_party(), &request_id, "yes").await;
    ...
    let (status, body) = v.resolve(v.oracle_party(), &request_id, "no").await;
    assert_eq!(status, StatusCode::CONFLICT);
...
async fn unmatched_leg_fails_request_and_releases_every_reservation() {
    ...
    v.quote_ok(mm, &request_id, &leg_a, 5_000, 1_000, v.at(600)).await;
    v.quote_ok(mm, &request_id, &leg_c, 5_000, 1_000, v.at(600)).await;
